...
import { Expose, Transform, TransformFnParams, Type as TransformType } from 'class-transformer';
import {
  IsArray,
  isBoolean,
  isBooleanString,
...
type PropertyType = Type<unknown> | (new (...args: any[]) => any) | Record<string, any> | 'file';
 ...
 */
export function ApiPropertyExtended(options: DtoPropertyOptions | undefined): PropertyDecorator {
 ...
 */
export function ValidateTransform(options: DtoPropertyOptions | undefined) {
  if (options === undefined || _.isEmpty(options)) {
  ...
  const decorators: PropertyDecorator[] = [Expose(), ValidateIf(() => isEmpty(validateGroup))];
...
function PropertyDto(options?: DtoPropertyOptions) {
  return applyDecorators(ApiPropertyExtended(options), ValidateTransform(options));
}
